(()=>{
  ...
  if(header)header.innerHTML=`<div class="container header-inner"><button class="menu-toggle" type="button" aria-controls="mobile-nav-drawer" aria-expanded="false" aria-label="Navigation öffnen"><span aria-hidden="true">☰</span></button><a class="brand" href="${path('index.html')}" aria-label="Bavaria AeroTech Startseite"><img src="${path('assets/brand/logo-horizontal.svg')}" width="390" height="82" alt="Bavaria AeroTech Solutions"></a><nav class="site-nav desktop-nav" aria-label="Hauptnavigation">${links}</nav><div class="header-actions"><div class="lang-switch" aria-label="Sprache"><button data-lang="de" aria-pressed="true">DE</button><button data-lang="en" aria-pressed="false">EN</button><button data-lang="uk" aria-pressed="false">UA</button></div><a class="btn btn--primary" href="${path('projektanfrage.html')}" data-i18n="nav.inquiry">Projektanfrage senden</a></div></div>`;
